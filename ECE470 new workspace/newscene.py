...
for i in range(0,6):
    errorCode, ur3_1_handle[i] = sim.simxGetObjectHandle(
        clientID,"UR3_joint"+str(i+100*i),sim.simx_opmode_oneshot_wait)
...
def getPos_2():
    ...
    returnCode, ur3_1_position = sim.simxGetObjectPosition(
    clientID, UR3_1_handle, -1, sim.simx_opmode_oneshot_wait)
 
    for i in range(0,6):
                         returnCode, redposition[i] = sim.simxGetObjectPosition(
                         clientID, red_ball_handle[i], -1, sim.simx_opmode_oneshot_wait)

                         redposition[i] = np.array(redposition[i])-np.array(ur3position)
    ...
    pinkposition = np.array(pinkposition)-np.array(ur3position)
    ...
    blackposition = np.array(blackposition)-np.array(ur3position)
    ...
    for i in range(0,6):
                        ...
                        clientID, red_des_handle[i], -1, sim.simx_opmode_oneshot_wait)
                        reddes[i] = np.reshape(reddes[i],(3,2))
                        reddes[i] = np.array([reddes[i][0],
                                              reddes[i][1],
                                              reddes[i][2]+0.051+0.05])
                        reddes[i] = reddes[i] - np.reshape(ur3position,(3,1))
    ...
    pinkdes = pinkdes - np.reshape(ur3position,(3,1))
    ...
    blackdes = blackdes - np.reshape(ur3position,(3,1))
    print ('Got Ball desitination')

    return ur3_1_position, redposition[i], pinkposition, blackposition, reddes[i], pinkdes, blackdes
